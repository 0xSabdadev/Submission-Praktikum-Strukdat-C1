/*********************************************/
/* Program   : list1.c                                      */
/* Deskripsi : file fungsi dan prosedur modul list          */
...
/*F.S. menghasilkan alamat elemen terakhir (Last) dan sebelumnya (Prec)*/
    ...
    if(ListEmpty(*L)){
        ...
        PrecLast = Nil;
        Laste = First(*L);
        while(next(Laste) != Nil){
            PrecLast = Laste;
            Laste = next(Laste);
        }
        printf("\nAlamat elemen terakhir : %d\n", Laste);
        printf("\nAlamat elemen sebelumnya : %d\n", PrecLast);
    }
}
...
/*Algoritma*/
    P = First(L);
    if(P == Nil){ //Apakah next dari P ada atau tidak (null)?
        printf("list Kosong\n"); //Print "list kosong" jika diketahui kosong
    }
    else{
        while(next(P) != Nil){ //Jika next dari P tidak (null)
            printf("%d,", info(P)); //Menampilkan isi list P
            P = next(P); //Merujuk isi list pada address berikutnya (looping)
        }
        printf("%d\n", info(P)); //Menampilkan isi list pada address setelah P
    }
}
...
/*Kamus Lokal*/
    ...
    address P;
/*Algoritma*/
    counter = 0; //Address awal/looping awal
    if(!ListEmpty(L)){ //Jika list tidak kosong/null
        P = First(L); //Merujuk pada nilai pertama list
        while(P != Nil){ //Jika p tidak kosong
            counter++; //Merujuk ke address berikutnya
            P = next(P); //Memanggil list P address berikutnya
        }
    }
    return counter; //looping counter++ hingga list terbaca kosong atau sudah selesai sampai address terakhir
}
